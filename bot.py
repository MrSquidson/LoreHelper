...
# Load .env file which contains our static token
load_dotenv()
...
# Sync command - Sync Current cmd tree.
@bot.tree.command(
    name="sync_cmd",
...
async def randact():
    ...
    while randAct == True:
        ...
        if chance == 0: # Makes the bot activity say that it's "Listening to Stories"
            await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name="Stories"))
            print('Activity set to: Listening to Stories')
            randAct = False
        elif chance == 1: # Makes the bot activity as that it's "Watching History Unfold"
            await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="History Unfold"))
            print('Activity set to: Watching hisotry')
            randAct = False
...
async def on_command_error(interaction: discord.Interaction, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        await interaction.response.send_message("You dont have all the required permissions :angry:")
